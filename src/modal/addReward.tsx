...
export default function ModalReward({ visible, onClose, onSave }: ModalComponentProps) {
  ...
  const handleAddReward = async () => {
    if (title && gold) {
      try {
        // Recupera o userID do SecureStore
        ...
        Alert.alert('Erro ao criar recompensa', error.response?.data?.message || 'Erro desconhecido.');
      }
    } else {
    ...
    <Modal animationType="fade" transparent={true} visible={visible} onRequestClose={onClose}>
      <Pressable onPress={onClose} className="flex-1 justify-center items-center bg-black/60">
        <Pressable className="w-90 bg-neutral-800 p-6 rounded-xl" onPress={() => {}}>
          <Text className="text-white text-2xl font-semibold mb-6">Adicionar Nova Recompensa</Text>
